...
#ifdef SPEC
#define strdup spec_strdup
char * spec_strdup(const char*);

#ifdef SPEC_WINDOWS
#define STRCMP(x,y) _stricmp(x,y)
#define STRNCMP(x,y,n) _strnicmp(x,y,n)
#else
...
static ALWAYS_INLINE void x264_predictor_roundclip( int16_t (*dst)[2], int16_t (*mvc)[2], int i_mvc, int mv_x_min, int mv_x_max, int mv_y_min, int mv_y_max )
{
...
static const signed char slice_type_to_char[] = { 'P', 'B', 'I', 'S', 'S' };
...
#define map_col_to_list0(col) h->mb.m_map_col_to_list0[(col)+2]
        int8_t  m_map_col_to_list0[18];
        int ref_blind_dupe; /* The index of the blind reference frame duplicate. */
        int8_t m_deblock_ref_table[32+2];
#define deblock_ref_table(x) h->mb.m_deblock_ref_table[(x)+2]
    } mb;
