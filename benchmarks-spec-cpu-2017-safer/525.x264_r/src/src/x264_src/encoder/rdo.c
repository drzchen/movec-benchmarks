...
#ifdef SPEC
#define VOID 
#define x264_macroblock_write_cavlc  static void x264_macroblock_size_cavlc
#else
...
#ifdef SPEC
#define x264_macroblock_write_cabac  static void x264_macroblock_size_cabac
#else
...
#define COPY_CABAC h->mc.memcpy_aligned( &cabac_tmp.f8_bits_encoded, &h->cabac.f8_bits_encoded, \
        offsetof(x264_cabac_t,state) - offsetof(x264_cabac_t,f8_bits_encoded) ); \
                   h->mc.memcpy_aligned( &cabac_tmp.state, &h->cabac.state, \
        sizeof(x264_cabac_t) - offsetof(x264_cabac_t,state) )
                                 ...
                                 int i_ctxBlockCat, int i_lambda2, int b_ac, int dc, int i_coefs, int idx )
{
    ...
    for( i = i_coefs-1; i >= b_ac; i-- )
        if( (unsigned int)(dct[zigzag[i]] * (dc?quant_mf[0]>>1:quant_mf[zigzag[i]]) + f-1) >= (unsigned int) 2*f )
            break;
...
static const uint8_t x264_zigzag_scan2[4] = {0,1,2,3};
