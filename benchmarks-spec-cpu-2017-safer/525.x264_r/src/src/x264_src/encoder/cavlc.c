...
#if defined(SPEC)
    { 1, 255, 255, 255, 2, 255, 255, 255, 3 }  /* D_16x16 */
#else
...
static int f_block_residual_write_cavlc( x264_t *h, int i_ctxBlockCat, int16_t *l, int nC )
{
    bs_t *s = &h->out.bs;
...
{\
    ...
    if( !*nnz )\
        ...
        *nnz = f_block_residual_write_cavlc(h,cat,l,nC);\
}
