...
void molio_free()
{
  ...
  for (ap = atab, a = 0; a < ATAB_SIZE; a++, ap++)
    if(ap->a_atomname) free(ap->a_atomname);
  if(res.r_resname) free(res.r_resname);
  if(res.r_resid)   free(res.r_resid);
}
        ...
        if (strncmp("ATOM", line, 4) == 0 ||
            ...
            *np1 = '\0';
            ...
            if (aname[0] == '1' || aname[0] == '2' || aname[0] == '3') {
                temp = aname[0];
                for (i = 0; i < (int)strlen(aname) - 1; i++)
                    aname[i] = aname[i + 1];
                aname[i] = temp;
            }
...
static void mk_brook_aname(char aname[], char name[], char rname[])
{
    ...
    if (!strncmp(aname, " H", 2) &&
        ...
        (!strncmp(aname, " HB", 3) ||
#if defined(SPEC)
         (strncmp(rname, "PHE", 3) && strncmp(rname, "TYR", 3) && strncmp(rname, "TRP", 3)
#else
