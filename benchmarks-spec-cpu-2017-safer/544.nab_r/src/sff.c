...
#if defined(SPEC)
/* Some compilation environments sometimes hide the definition of strdup()
 * in standards mode.
 */
# undef strdup
# define strdup spec_strdup
static char *spec_strdup(const char *s1) {
  char *new = (char *)malloc(strlen(s1) + 1);
  if (new)
    new = strcpy(new, s1);
...
#if !defined(SPEC) || defined(SPEC_VERBOSE_OUTPUT)
      if (mytaskid == 0) {
         if (nstep % ntpr_md == 0 || nstep == 1 || nstep == maxstep) {
            fprintf(nabout,
